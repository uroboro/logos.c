// https://www.rosettacode.org/wiki/Read_a_specific_line_from_a_file#C
#include <unistd.h>
#include <sys/types.h>
#include <sys/mman.h>
#include <sys/stat.h>
 ...
 * checked for error.
*/
int read_file_line(const char *path, int line_no, char * buffer)
{
	struct stat s;
	char *buf;
	off_t start = -1, end = -1;
	...
	if (start >= s.st_size || start < 0) {
		...
		*/
		...
		memcpy(buffer, buf + start, end - start - 1);
	}
